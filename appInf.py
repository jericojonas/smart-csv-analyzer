...
def get_column_types(df, threshold=0.7):  # threshold: minimal 70% bisa dikonversi ke float
    numeric_cols = df.select_dtypes(include=['number']).columns.tolist()
    
    # Cek kolom bertipe object apakah bisa dikonversi ke float
    for col in df.select_dtypes(include=['object']).columns:
        converted = df[col].apply(convert_to_float_safe)
        success_ratio = converted.notnull().mean()
        if success_ratio >= threshold:
            numeric_cols.append(col)
            # konversi kolom tersebut menjadi float di df
            df[col] = converted
...
def show_full_summary(df, kolom):
    ...
    default_index = kolom_lain.index("Username") if "Login ID" in kolom_lain else 0
